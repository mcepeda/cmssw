...
void GEMUnpackingModule::produce(edm::Event & ev, const edm::EventSetup& es)
{
  ...
  if(!allFEDRawData.isValid()) { LogTrace("") <<"There is no collection, there should be an empty one!"<<std::endl; return;} 
    ...
    */
	...
	switch (line){
        ...
        if ( line-5 > 0 &&  line < 5+(int)sumVFAT*4+1 ){ 
          if ((line-5)%4 == 1) ivfat++;
          //std::cout <<" line "<< line <<" vfat in "<< (line-5)%4 <<" ivfat "<< ivfat << std::endl;
		...
		*/
		...
		if (ChipID == 0xe7b){
		  etaP=1;
		  strip=chan;
		}else if(ChipID == 0xe5b){
		  etaP=1;
		  strip=128+chan;
		}else if(ChipID == 0xe8b){
		  etaP=1;
		  strip=128*2+chan;
		}else if(ChipID == 0xaaf){
		  etaP=2;
		  strip=chan;
		}else if(ChipID == 0xe98){
		  etaP=2;
		  strip=128+chan;
		}
		// this is a single digi, where we should give in input the strip and bx relative to trigger that is always 0 for VFAT2
                /*
		std::cout << " etaP " << etaP << std::hex << " ChipID 0x" << ChipID << " BX 0x" << bx << std::dec << "  chan " << chan 
