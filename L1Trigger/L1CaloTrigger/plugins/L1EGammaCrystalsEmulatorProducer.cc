   ...
   for (int ii=0; ii<48; ++ii){
       for (int jj=0; jj<2; ++jj){
		   ...
		   L1EGXtalClusterEmulator->push_back(cluster);
                   ...
                   int quality = (standaloneWP) + (looseL1TkMatchWP<<1) + (photonWP<<2);
                        std::cout<<standaloneWP<<"   "<<looseL1TkMatchWP<<"   "<<photonWP<<"   "<<quality<<std::endl;
                   ...
                   //l1t::EGamma egcand=l1t::EGamma(p4calibrated, p4calibrated.pt(), p4calibrated.eta(), p4calibrated.phi(),quality,1 );
                   //L1EGammaCollectionBXVEmulator->push_back(0,egcand);
